...
> Reduce costs for **[AI SYSTEM]** currently spending **$[AMOUNT]/month** on **[COST DRIVERS]**. Guide me through: (1) **Cost analysis**—what's the unit economics (cost per prediction/user/request)? Where is money being wasted? (2) **Quick wins**—what immediate optimizations (right-sizing, spot instances, caching) can cut costs this week? (3) **Model efficiency**—should I use quantization, model routing, or smaller models? What's the quality tradeoff? (4) **LLM optimization**—how to reduce tokens, implement caching, and route to cheaper models? Provide a savings roadmap with effort/impact matrix, projected monthly savings, and quality safeguards.
