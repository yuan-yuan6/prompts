---
category: sales-marketing
last_updated: 2025-11-09
related_templates:
- content-creation/creative-writing-framework.md
- content-creation/video-production-pipeline.md
tags:
- ai-ml
- automation
- communication
- design
- documentation
- marketing
- research
title: Ad Copy Template
use_cases:
- Write A/B test variations for Facebook/Google ads (15-30 min for 3-5 versions)
- Create platform-specific ad copy for multi-channel campaign launch
- Develop compelling headlines and CTAs that improve click-through rates
industries:
- healthcare
- manufacturing
- retail
type: template
difficulty: intermediate
slug: ad-copy
---

# Ad Copy Template

## Purpose
Create compelling advertising copy for digital, print, and broadcast media that drives engagement, conversions, and brand awareness across multiple channels and platforms.

## Quick Ad Copy Prompt
Create [AD_TYPE] (Google/Facebook/LinkedIn/display) ads for [BRAND_NAME] targeting [TARGET_AUDIENCE] with [CAMPAIGN_OBJECTIVE] (awareness/traffic/conversions) goal. Product/service: [PRODUCT_DESCRIPTION]. Key benefit: [PRIMARY_BENEFIT]. Write [VARIATION_COUNT] headline variations (problem-focused, benefit-focused, curiosity-driven). Include [CTA_TYPE] CTAs and [PROOF_ELEMENT] (testimonial/stat/trust badge). Target [CTR_TARGET]% CTR and [CONVERSION_TARGET]% conversion rate. Character limits: [HEADLINE_LIMIT] headline, [DESCRIPTION_LIMIT] description.

## Quick Start

Need ad copy that stops the scroll and drives clicks? Here's your battle plan:

1. **Match platform to message** - Google Ads for intent-driven searches, Facebook for discovery, LinkedIn for B2B
2. **Lead with outcome, not process** - "Cut meeting prep from 2 hours to 15 minutes" beats "AI-powered scheduling tool"
3. **Write 3 headline variations** - Problem-focused, benefit-focused, and curiosity-driven
4. **Make CTAs specific** - "Start Free Trial" outperforms "Learn More" every time
5. **Build in proof** - Add one trust element: testimonial, stat, or brand logo

**Try this:** Pick one platform from the examples below (start with Google Ads if you're intent-focused or Facebook if you're discovery-driven). Write 3 headline variations and 2 description options, then A/B test them with small budgets. Let the data tell you what resonates, then double down on winners.

## Template

```
You are an advertising copywriter expert. Create [AD_TYPE] for [BRAND_NAME] targeting [TARGET_AUDIENCE] with a focus on [CAMPAIGN_OBJECTIVE].

ADVERTISING BRIEF:
Campaign Overview:
- Brand name: [BRAND_NAME]
- Product/service: [PRODUCT_SERVICE]
- Campaign name: [CAMPAIGN_NAME]
- Campaign objective: [CAMPAIGN_OBJECTIVE]
- Target audience: [TARGET_AUDIENCE]
- Key message: [KEY_MESSAGE]
- Budget range: [BUDGET_RANGE]
- Campaign duration: [CAMPAIGN_DURATION]
- Success metrics: [SUCCESS_METRICS]

### Brand Guidelines
- Brand voice: [BRAND_VOICE] (Professional/Friendly/Edgy/Authoritative)
- Brand tone: [BRAND_TONE] (Conversational/Formal/Playful/Urgent)
- Brand personality: [BRAND_PERSONALITY]
- Brand values: [BRAND_VALUES]
- Unique selling proposition: [UNIQUE_SELLING_PROPOSITION]
- Brand positioning: [BRAND_POSITIONING]
- Competitor differentiation: [COMPETITOR_DIFFERENTIATION]
- Brand guidelines: [BRAND_GUIDELINES]

### Audience Analysis
- Primary demographic: [PRIMARY_DEMOGRAPHIC]
- Secondary demographic: [SECONDARY_DEMOGRAPHIC]
- Psychographic profile: [PSYCHOGRAPHIC_PROFILE]
- Pain points: [AUDIENCE_PAIN_POINTS]
- Motivations: [AUDIENCE_MOTIVATIONS]
- Media consumption: [MEDIA_CONSUMPTION]
- Decision-making process: [DECISION_MAKING_PROCESS]
- Preferred communication style: [COMMUNICATION_STYLE]

### DIGITAL ADVERTISING COPY

### Search Engine Marketing
### Google Ads Copy
Headline 1: [HEADLINE_1] (30 characters max)
Headline 2: [HEADLINE_2] (30 characters max)
Headline 3: [HEADLINE_3] (30 characters max)
Description 1: [DESCRIPTION_1] (90 characters max)
Description 2: [DESCRIPTION_2] (90 characters max)
Path 1: [PATH_1] (15 characters max)
Path 2: [PATH_2] (15 characters max)

### Ad Extensions
- Sitelink 1: [SITELINK_1] - Description: [SITELINK_1_DESC]
- Sitelink 2: [SITELINK_2] - Description: [SITELINK_2_DESC]
- Sitelink 3: [SITELINK_3] - Description: [SITELINK_3_DESC]
- Callout 1: [CALLOUT_1]
- Callout 2: [CALLOUT_2]
- Callout 3: [CALLOUT_3]
- Structured snippet: [STRUCTURED_SNIPPET]

### Bing Ads Variations
- Bing Headline 1: [BING_HEADLINE_1]
- Bing Headline 2: [BING_HEADLINE_2]
- Bing Description: [BING_DESCRIPTION]
- Bing Display URL: [BING_DISPLAY_URL]

### Social Media Advertising
Facebook/Meta Ads:
- Primary text: [FB_PRIMARY_TEXT] (125 characters recommended)
- Headline: [FB_HEADLINE] (27 characters recommended)
- Description: [FB_DESCRIPTION] (27 characters recommended)
- Call-to-action: [FB_CTA]
- Link description: [FB_LINK_DESCRIPTION]

### Instagram Ads
- Instagram caption: [IG_CAPTION] (2200 characters max)
- Instagram hashtags: [IG_HASHTAGS]
- Story text overlay: [IG_STORY_TEXT]
- IGTV title: [IGTV_TITLE]
- Reel description: [REEL_DESCRIPTION]

### LinkedIn Ads
- LinkedIn headline: [LI_HEADLINE] (150 characters max)
- LinkedIn description: [LI_DESCRIPTION] (300 characters max)
- Sponsored content: [LI_SPONSORED_CONTENT]
- Message ad copy: [LI_MESSAGE_AD]
- Dynamic ad personalization: [LI_DYNAMIC_PERSONALIZATION]

Twitter/X Ads:
- Tweet copy: [TWITTER_COPY] (280 characters max)
- Promoted tweet: [PROMOTED_TWEET]
- Twitter card title: [TWITTER_CARD_TITLE]
- Twitter card description: [TWITTER_CARD_DESCRIPTION]
- Hashtag strategy: [TWITTER_HASHTAGS]

### TikTok Ads
- TikTok caption: [TIKTOK_CAPTION] (100 characters recommended)
- Spark ads copy: [SPARK_ADS_COPY]
- Brand takeover text: [BRAND_TAKEOVER_TEXT]
- TopView ad copy: [TOPVIEW_AD_COPY]
- In-feed ad copy: [INFEED_AD_COPY]

### YouTube Ads
- Video title: [YOUTUBE_TITLE] (100 characters max)
- Video description: [YOUTUBE_DESCRIPTION] (5000 characters max)
- Bumper ad script: [BUMPER_AD_SCRIPT] (6 seconds)
- TrueView ad script: [TRUEVIEW_SCRIPT]
- Discovery ad headline: [DISCOVERY_HEADLINE]
- Discovery ad description: [DISCOVERY_DESCRIPTION]

### PRINT ADVERTISING COPY

### Magazine Advertisements
Full-Page Ad:
- Main headline: [MAGAZINE_MAIN_HEADLINE]
- Subheadline: [MAGAZINE_SUBHEADLINE]
- Body copy: [MAGAZINE_BODY_COPY]
- Call-to-action: [MAGAZINE_CTA]
- Contact information: [MAGAZINE_CONTACT]
- Legal disclaimer: [MAGAZINE_DISCLAIMER]
- Tagline: [MAGAZINE_TAGLINE]

Half-Page Ad:
- Condensed headline: [HALF_PAGE_HEADLINE]
- Key benefit statement: [HALF_PAGE_BENEFIT]
- Bullet points: [HALF_PAGE_BULLETS]
- Strong CTA: [HALF_PAGE_CTA]
- Essential contact: [HALF_PAGE_CONTACT]

Quarter-Page Ad:
- Impact headline: [QUARTER_PAGE_HEADLINE]
- Single benefit: [QUARTER_PAGE_BENEFIT]
- Direct CTA: [QUARTER_PAGE_CTA]
- Website/phone: [QUARTER_PAGE_CONTACT]

### Newspaper Advertisements
### Display Ad
- Attention-grabbing headline: [NEWSPAPER_HEADLINE]
- Compelling offer: [NEWSPAPER_OFFER]
- Benefit-focused copy: [NEWSPAPER_BENEFITS]
- Urgency element: [NEWSPAPER_URGENCY]
- Clear CTA: [NEWSPAPER_CTA]
- Business info: [NEWSPAPER_BUSINESS_INFO]

### Classified Ad
- Classified headline: [CLASSIFIED_HEADLINE] (25-35 characters)
- Classified body: [CLASSIFIED_BODY] (50-100 words)
- Contact method: [CLASSIFIED_CONTACT]
- Price/offer: [CLASSIFIED_OFFER]

### Direct Mail
### Postcard Copy
- Front headline: [POSTCARD_FRONT_HEADLINE]
- Front subhead: [POSTCARD_FRONT_SUBHEAD]
- Back headline: [POSTCARD_BACK_HEADLINE]
- Offer details: [POSTCARD_OFFER_DETAILS]
- Response mechanism: [POSTCARD_RESPONSE]
- Expiration date: [POSTCARD_EXPIRATION]

### Letter Copy
- Envelope teaser: [ENVELOPE_TEASER]
- Letter opening: [LETTER_OPENING]
- Problem identification: [LETTER_PROBLEM]
- Solution presentation: [LETTER_SOLUTION]
- Proof/credibility: [LETTER_PROOF]
- Offer details: [LETTER_OFFER]
- Call-to-action: [LETTER_CTA]
- P.S. message: [LETTER_PS]

### BROADCAST ADVERTISING COPY

### Radio Commercials
15-Second Radio:
- Opening hook: [RADIO_15_HOOK] (3-5 seconds)
- Key message: [RADIO_15_MESSAGE] (6-8 seconds)
- CTA and brand: [RADIO_15_CTA] (4-6 seconds)

30-Second Radio:
- Attention grabber: [RADIO_30_OPENER] (5-7 seconds)
- Problem/benefit: [RADIO_30_PROBLEM_BENEFIT] (10-12 seconds)
- Solution/offer: [RADIO_30_SOLUTION] (8-10 seconds)
- Call-to-action: [RADIO_30_CTA] (5-7 seconds)

60-Second Radio:
- Story opening: [RADIO_60_STORY_OPENING] (10-15 seconds)
- Problem development: [RADIO_60_PROBLEM] (15-20 seconds)
- Solution introduction: [RADIO_60_SOLUTION] (15-20 seconds)
- Offer presentation: [RADIO_60_OFFER] (10-15 seconds)
- Strong close: [RADIO_60_CLOSE] (5-10 seconds)

### Television Commercials
15-Second TV:
- Visual concept: [TV_15_VISUAL_CONCEPT]
- Audio script: [TV_15_AUDIO_SCRIPT]
- Super text: [TV_15_SUPER_TEXT]
- End card: [TV_15_END_CARD]

30-Second TV:
- Scene 1: [TV_30_SCENE_1] (8-10 seconds)
- Scene 2: [TV_30_SCENE_2] (8-10 seconds)
- Scene 3: [TV_30_SCENE_3] (8-10 seconds)
- Close/CTA: [TV_30_CLOSE] (4-6 seconds)

Streaming/OTT Ads:
- Pre-roll script: [PREROLL_SCRIPT] (15-30 seconds)
- Mid-roll script: [MIDROLL_SCRIPT] (15-30 seconds)
- Post-roll script: [POSTROLL_SCRIPT] (15-30 seconds)
- Connected TV copy: [CONNECTED_TV_COPY]
- Addressable TV copy: [ADDRESSABLE_TV_COPY]

### OUTDOOR ADVERTISING COPY

### Billboard Copy
### Highway Billboard
- Main message: [BILLBOARD_MAIN_MESSAGE] (7 words max)
- Secondary message: [BILLBOARD_SECONDARY] (3-5 words)
- Brand identifier: [BILLBOARD_BRAND]
- Contact info: [BILLBOARD_CONTACT] (website/phone)

### Transit Billboard
- Transit headline: [TRANSIT_HEADLINE] (5-8 words)
- Transit subhead: [TRANSIT_SUBHEAD] (8-12 words)
- Visual description: [TRANSIT_VISUAL]
- Brand/contact: [TRANSIT_BRAND_CONTACT]

### Digital Billboards
- Digital message 1: [DIGITAL_MESSAGE_1] (6-8 seconds display)
- Digital message 2: [DIGITAL_MESSAGE_2] (6-8 seconds display)
- Digital message 3: [DIGITAL_MESSAGE_3] (6-8 seconds display)
- Rotation timing: [DIGITAL_ROTATION_TIMING]

### Transit Advertising
Bus/Train Interior:
- Interior headline: [INTERIOR_HEADLINE]
- Interior body copy: [INTERIOR_BODY_COPY]
- Interior CTA: [INTERIOR_CTA]
- QR code message: [QR_CODE_MESSAGE]

### Exterior Transit
- Exterior message: [EXTERIOR_MESSAGE] (6 words max)
- Route relevance: [ROUTE_RELEVANCE]
- Contact method: [EXTERIOR_CONTACT]

### SPECIALTY ADVERTISING COPY

### Native Advertising
Article-Style Native:
- Native headline: [NATIVE_HEADLINE]
- Native introduction: [NATIVE_INTRO]
- Native body paragraphs: [NATIVE_BODY]
- Native conclusion: [NATIVE_CONCLUSION]
- Native CTA: [NATIVE_CTA]
- Disclosure statement: [NATIVE_DISCLOSURE]

### Video Native
- Video native script: [VIDEO_NATIVE_SCRIPT]
- Video native CTA: [VIDEO_NATIVE_CTA]
- Video description: [VIDEO_NATIVE_DESCRIPTION]

### Influencer Marketing
### Influencer Brief
- Campaign hashtags: [INFLUENCER_HASHTAGS]
- Key talking points: [INFLUENCER_TALKING_POINTS]
- Must-mention elements: [INFLUENCER_MUST_MENTION]
- Disclosure requirements: [INFLUENCER_DISCLOSURE]
- Content guidelines: [INFLUENCER_GUIDELINES]
- CTA suggestions: [INFLUENCER_CTA_SUGGESTIONS]

Micro-Influencer Copy:
- Micro-influencer script: [MICRO_INFLUENCER_SCRIPT]
- Authenticity guidelines: [AUTHENTICITY_GUIDELINES]
- Product integration: [PRODUCT_INTEGRATION]
- Story elements: [STORY_ELEMENTS]

### EMAIL MARKETING INTEGRATION

### Email Headlines
- Welcome series subject: [WELCOME_EMAIL_SUBJECT]
- Promotional subject: [PROMOTIONAL_EMAIL_SUBJECT]
- Newsletter subject: [NEWSLETTER_SUBJECT]
- Abandoned cart subject: [ABANDONED_CART_SUBJECT]
- Re-engagement subject: [REENGAGEMENT_SUBJECT]

### Email Preview Text
- Welcome preview: [WELCOME_PREVIEW_TEXT]
- Promotional preview: [PROMOTIONAL_PREVIEW_TEXT]
- Newsletter preview: [NEWSLETTER_PREVIEW_TEXT]
- Cart abandonment preview: [CART_ABANDONMENT_PREVIEW]

### CAMPAIGN VARIATIONS

A/B Testing Variations:
### Headline Tests
- Headline variation A: [HEADLINE_A]
- Headline variation B: [HEADLINE_B]
- Headline variation C: [HEADLINE_C]
- Test hypothesis: [HEADLINE_TEST_HYPOTHESIS]
- Success metric: [HEADLINE_SUCCESS_METRIC]

### CTA Tests
- CTA variation A: [CTA_A]
- CTA variation B: [CTA_B]
- CTA variation C: [CTA_C]
- CTA test rationale: [CTA_TEST_RATIONALE]

### Seasonal Adaptations
- Spring campaign copy: [SPRING_COPY]
- Summer campaign copy: [SUMMER_COPY]
- Fall campaign copy: [FALL_COPY]
- Winter campaign copy: [WINTER_COPY]
- Holiday adaptations: [HOLIDAY_ADAPTATIONS]

### Demographic Variations
- Gen Z messaging: [GEN_Z_MESSAGING]
- Millennial messaging: [MILLENNIAL_MESSAGING]
- Gen X messaging: [GEN_X_MESSAGING]
- Boomer messaging: [BOOMER_MESSAGING]
- Cultural adaptations: [CULTURAL_ADAPTATIONS]

### PERFORMANCE OPTIMIZATION

### Conversion Optimization
- Primary conversion goal: [PRIMARY_CONVERSION_GOAL]
- Secondary conversion goals: [SECONDARY_CONVERSION_GOALS]
- Conversion tracking setup: [CONVERSION_TRACKING]
- Landing page alignment: [LANDING_PAGE_ALIGNMENT]
- Message matching: [MESSAGE_MATCHING]
- Funnel optimization: [FUNNEL_OPTIMIZATION]

### Engagement Optimization
- Engagement metrics: [ENGAGEMENT_METRICS]
- Content relevance: [CONTENT_RELEVANCE]
- Emotional triggers: [EMOTIONAL_TRIGGERS]
- Social proof elements: [SOCIAL_PROOF_ELEMENTS]
- Urgency creation: [URGENCY_CREATION]
- Personalization elements: [PERSONALIZATION_ELEMENTS]

### COMPLIANCE & LEGAL

### Advertising Standards
- Industry regulations: [INDUSTRY_REGULATIONS]
- Platform policies: [PLATFORM_POLICIES]
- Truth in advertising: [TRUTH_IN_ADVERTISING]
- Substantiation requirements: [SUBSTANTIATION_REQUIREMENTS]
- Disclosure requirements: [DISCLOSURE_REQUIREMENTS]
- Accessibility compliance: [ACCESSIBILITY_COMPLIANCE]

### Legal Disclaimers
- General disclaimer: [GENERAL_DISCLAIMER]
- Offer disclaimer: [OFFER_DISCLAIMER]
- Medical disclaimer: [MEDICAL_DISCLAIMER]
- Financial disclaimer: [FINANCIAL_DISCLAIMER]
- Geographic limitations: [GEOGRAPHIC_LIMITATIONS]
- Age restrictions: [AGE_RESTRICTIONS]

### DESIGN OUTPUT
[Generate comprehensive advertising copy with all specified elements]

Campaign: [FINAL_CAMPAIGN_NAME]
Ad Type: [FINAL_AD_TYPE]

[COMPLETE_AD_COPY_SOLUTION]

---

### Copy Deliverables
- Digital ad variations: [DIGITAL_AD_COUNT]
- Print ad copies: [PRINT_AD_COUNT]
- Broadcast scripts: [BROADCAST_SCRIPT_COUNT]
- Social media posts: [SOCIAL_MEDIA_COUNT]
- Email subject lines: [EMAIL_SUBJECT_COUNT]
- A/B test variations: [AB_TEST_COUNT]

OUTPUT: Deliver comprehensive ad copy with:
1. Platform-optimized digital copy
2. Professional print advertisements
3. Engaging broadcast scripts
4. Social media content
5. Email marketing copy
6. A/B testing variations
7. Compliance documentation
```

## Variables
[All 400+ variables for comprehensive ad copy across digital, print, and broadcast media]

## Usage Examples

## Best Practices

1. **Lead with the outcome, not the process** - "Lost 20 lbs" beats "Science-backed weight loss program"
2. **Test multiple angles for same offer** - Problem-focused vs. benefit-focused vs. fear-based can all work for different audiences
3. **Front-load the value in headlines** - Users decide in 2 seconds; buried benefits = lost clicks
4. **Quantify whenever possible** - "Save time" is vague; "Cut meeting prep from 2 hours to 15 minutes" sells
5. **Match sophistication to audience awareness** - Aware audiences need proof, unaware need education first

## Tips for Success

- Break complex tasks into manageable steps with clear milestones
- Set realistic timelines that account for dependencies and constraints
- Allocate sufficient resources including time, budget, and personnel
- Use templates and frameworks to ensure consistency and quality
- Seek feedback from users and stakeholders throughout the process
- Build in checkpoints to assess progress and make adjustments
- Maintain quality standards while remaining practical and efficient
- Document lessons learned for future reference and improvement
- Foster collaboration across teams and departments
- Stay current with industry best practices and emerging trends
### Example 1: SaaS Product Launch
```
AD_TYPE: "Digital advertising campaign"
BRAND_NAME: "ProductivityMax"
TARGET_AUDIENCE: "Small business owners and entrepreneurs"
CAMPAIGN_OBJECTIVE: "Drive free trial signups"
BRAND_VOICE: "Professional yet approachable"
KEY_MESSAGE: "Streamline your workflow and boost productivity"
```

### Example 2: E-commerce Fashion Brand
```
AD_TYPE: "Social media advertising campaign"
BRAND_NAME: "StyleForward"
TARGET_AUDIENCE: "Fashion-conscious millennials and Gen Z"
CAMPAIGN_OBJECTIVE: "Increase online sales and brand awareness"
BRAND_VOICE: "Trendy and aspirational"
KEY_MESSAGE: "Express your unique style with confidence"
```

### Example 3: Local Service Business
```
AD_TYPE: "Local advertising campaign"
BRAND_NAME: "QuickFix Home Services"
TARGET_AUDIENCE: "Homeowners in metropolitan area"
CAMPAIGN_OBJECTIVE: "Generate service bookings"
BRAND_VOICE: "Trustworthy and reliable"
KEY_MESSAGE: "Professional home services you can count on"
```



## Related Resources

### Complementary Templates

Enhance your workflow by combining this template with:

- **[Creative Writing Framework](creative-writing-framework.md)** - Complementary approaches and methodologies
- **[Video Production Pipeline](video-production-pipeline.md)** - Complementary approaches and methodologies

### Suggested Workflow

**Typical implementation sequence**:

1. Start with this template (Ad Copy Template)
2. Use [Creative Writing Framework](creative-writing-framework.md) for deeper analysis
3. Apply [Video Production Pipeline](video-production-pipeline.md) for execution
4. Iterate and refine based on results

### Explore More in This Category

Browse all **[creative/Marketing Creative](../../creative/Marketing Creative/)** templates for related tools and frameworks.

### Common Use Case Combinations

- **Write A/B test variations for Facebook/Google ads (15-30 min for 3-5 versions)**: Combine this template with related analytics and strategy frameworks
- **Create platform-specific ad copy for multi-channel campaign launch**: Combine this template with related analytics and strategy frameworks
- **Develop compelling headlines and CTAs that improve click-through rates**: Combine this template with related analytics and strategy frameworks

## Customization Options

1. **Media Types**
   - Digital advertising
   - Print advertising
   - Broadcast advertising
   - Outdoor advertising
   - Direct mail
   - Native advertising
   - Influencer marketing

2. **Industry Specialization**
   - Technology/SaaS
   - E-commerce/Retail
   - Healthcare
   - Financial services
   - Real estate
   - Automotive
   - Food & beverage

3. **Campaign Objectives**
   - Brand awareness
   - Lead generation
   - Sales conversion
   - App downloads
   - Event registration
   - Newsletter signups
   - Customer retention

4. **Audience Segments**
   - B2B professionals
   - Consumers by demographics
   - Geographic targeting
   - Interest-based targeting
   - Behavioral targeting
   - Lookalike audiences
   - Custom audiences