---
title: Brand Storytelling Template
category: creative/Marketing Creative
tags: [communication, creative, design, development, marketing, template]
use_cases:
  - Implementing create compelling brand narratives, voice, and personality that connect emotiona...
  - Project planning and execution
  - Strategy development
related_templates:
  - creative-writing-framework.md
  - video-production-pipeline.md
last_updated: 2025-11-09
---

# Brand Storytelling Template

## Purpose
Create compelling brand narratives, voice, and personality that connect emotionally with audiences and build lasting brand relationships across all touchpoints and communications.

## Template

```
You are a brand storytelling expert. Create [STORY_TYPE] for [BRAND_NAME] targeting [TARGET_AUDIENCE] with a focus on [STORYTELLING_OBJECTIVE].

BRAND STORYTELLING BRIEF:
Brand Foundation:
- Brand name: [BRAND_NAME]
- Industry/Category: [INDUSTRY_CATEGORY]
- Company stage: [COMPANY_STAGE] (Startup/Growth/Established/Legacy)
- Brand heritage: [BRAND_HERITAGE]
- Company mission: [COMPANY_MISSION]
- Company vision: [COMPANY_VISION]
- Core values: [CORE_VALUES]
- Brand purpose: [BRAND_PURPOSE]
- Business objectives: [BUSINESS_OBJECTIVES]

### Storytelling Context
- Story type: [STORY_TYPE] (Origin/Customer/Product/Vision/Values)
- Primary objective: [STORYTELLING_OBJECTIVE]
- Target audience: [TARGET_AUDIENCE]
- Emotional goal: [EMOTIONAL_GOAL]
- Behavioral goal: [BEHAVIORAL_GOAL]
- Story medium: [STORY_MEDIUM]
- Story length: [STORY_LENGTH]
- Distribution channels: [DISTRIBUTION_CHANNELS]
- Success metrics: [SUCCESS_METRICS]

### Market Context
- Competitive landscape: [COMPETITIVE_LANDSCAPE]
- Market position: [MARKET_POSITION]
- Category conventions: [CATEGORY_CONVENTIONS]
- Disruption opportunities: [DISRUPTION_OPPORTUNITIES]
- Cultural trends: [CULTURAL_TRENDS]
- Social climate: [SOCIAL_CLIMATE]
- Consumer sentiment: [CONSUMER_SENTIMENT]
- Market challenges: [MARKET_CHALLENGES]

### NARRATIVE DEVELOPMENT

### Story Architecture
### Central Narrative
- Story premise: [STORY_PREMISE]
- Central theme: [CENTRAL_THEME]
- Core message: [CORE_MESSAGE]
- Narrative arc: [NARRATIVE_ARC]
- Story conflict: [STORY_CONFLICT]
- Resolution/transformation: [RESOLUTION_TRANSFORMATION]
- Universal truth: [UNIVERSAL_TRUTH]
- Emotional payoff: [EMOTIONAL_PAYOFF]

### Origin Story
### Brand Genesis
- Founding moment: [FOUNDING_MOMENT]
- Founder background: [FOUNDER_BACKGROUND]
- Original problem: [ORIGINAL_PROBLEM]
- Inspiration source: [INSPIRATION_SOURCE]
- First solution: [FIRST_SOLUTION]
- Early challenges: [EARLY_CHALLENGES]
- Breakthrough moments: [BREAKTHROUGH_MOMENTS]
- Evolution journey: [EVOLUTION_JOURNEY]

### Company Mythology
- Founding legend: [FOUNDING_LEGEND]
- Origin symbolism: [ORIGIN_SYMBOLISM]
- Brand artifacts: [BRAND_ARTIFACTS]
- Historical milestones: [HISTORICAL_MILESTONES]
- Pivotal decisions: [PIVOTAL_DECISIONS]
- Cultural moments: [CULTURAL_MOMENTS]
- Legacy elements: [LEGACY_ELEMENTS]
- Continuation themes: [CONTINUATION_THEMES]

### Vision Story
### Future Narrative
- Future vision: [FUTURE_VISION]
- World transformation: [WORLD_TRANSFORMATION]
- Better tomorrow: [BETTER_TOMORROW]
- Innovation promise: [INNOVATION_PROMISE]
- Progress story: [PROGRESS_STORY]
- Change catalyst: [CHANGE_CATALYST]
- Aspirational elements: [ASPIRATIONAL_ELEMENTS]
- Hope messaging: [HOPE_MESSAGING]

Purpose-Driven Narrative:
- Social impact: [SOCIAL_IMPACT]
- Environmental commitment: [ENVIRONMENTAL_COMMITMENT]
- Community contribution: [COMMUNITY_CONTRIBUTION]
- Ethical stance: [ETHICAL_STANCE]
- Advocacy position: [ADVOCACY_POSITION]
- Change leadership: [CHANGE_LEADERSHIP]
- Responsibility narrative: [RESPONSIBILITY_NARRATIVE]
- Values demonstration: [VALUES_DEMONSTRATION]

### BRAND VOICE & PERSONALITY

### Voice Definition
### Brand Voice Characteristics
- Tone of voice: [TONE_OF_VOICE] (Authoritative/Friendly/Inspirational/Conversational)
- Communication style: [COMMUNICATION_STYLE]
- Language level: [LANGUAGE_LEVEL] (Simple/Sophisticated/Technical/Accessible)
- Vocabulary choices: [VOCABULARY_CHOICES]
- Sentence structure: [SENTENCE_STRUCTURE]
- Rhythm and pace: [RHYTHM_PACE]
- Emotional expression: [EMOTIONAL_EXPRESSION]
- Humor approach: [HUMOR_APPROACH]

### Voice Consistency
- Primary voice attributes: [PRIMARY_VOICE_ATTRIBUTES]
- Secondary voice traits: [SECONDARY_VOICE_TRAITS]
- Voice do's: [VOICE_DOS]
- Voice don'ts: [VOICE_DONTS]
- Context adaptations: [CONTEXT_ADAPTATIONS]
- Audience variations: [AUDIENCE_VARIATIONS]
- Channel modifications: [CHANNEL_MODIFICATIONS]
- Consistency guidelines: [CONSISTENCY_GUIDELINES]

### Brand Personality
### Personality Framework
- Primary personality traits: [PRIMARY_PERSONALITY_TRAITS]
- Secondary characteristics: [SECONDARY_CHARACTERISTICS]
- Personality archetype: [PERSONALITY_ARCHETYPE]
- Human characteristics: [HUMAN_CHARACTERISTICS]
- Behavioral patterns: [BEHAVIORAL_PATTERNS]
- Emotional tendencies: [EMOTIONAL_TENDENCIES]
- Relationship style: [RELATIONSHIP_STYLE]
- Value system: [PERSONALITY_VALUE_SYSTEM]

### Personality Expression
- Visual personality: [VISUAL_PERSONALITY]
- Verbal personality: [VERBAL_PERSONALITY]
- Behavioral personality: [BEHAVIORAL_PERSONALITY]
- Social personality: [SOCIAL_PERSONALITY]
- Cultural personality: [CULTURAL_PERSONALITY]
- Professional personality: [PROFESSIONAL_PERSONALITY]
- Personal personality: [PERSONAL_PERSONALITY]
- Situational adaptations: [SITUATIONAL_ADAPTATIONS]

### Character Development
### Brand Character
- Character description: [CHARACTER_DESCRIPTION]
- Character background: [CHARACTER_BACKGROUND]
- Character motivations: [CHARACTER_MOTIVATIONS]
- Character flaws: [CHARACTER_FLAWS]
- Character strengths: [CHARACTER_STRENGTHS]
- Character growth: [CHARACTER_GROWTH]
- Character relationships: [CHARACTER_RELATIONSHIPS]
- Character journey: [CHARACTER_JOURNEY]

### Persona Details
- Age/Generation: [PERSONA_AGE_GENERATION]
- Geographic origins: [GEOGRAPHIC_ORIGINS]
- Educational background: [EDUCATIONAL_BACKGROUND]
- Professional history: [PROFESSIONAL_HISTORY]
- Personal interests: [PERSONAL_INTERESTS]
- Values alignment: [VALUES_ALIGNMENT]
- Lifestyle choices: [LIFESTYLE_CHOICES]
- Social connections: [SOCIAL_CONNECTIONS]

### STORYTELLING FORMATS

### Customer Stories
Hero's Journey:
### Customer as Hero
- Customer challenge: [CUSTOMER_CHALLENGE]
- Call to action: [CUSTOMER_CALL_TO_ACTION]
- Initial resistance: [INITIAL_RESISTANCE]
- Meeting the guide: [MEETING_THE_GUIDE] (Brand role)
- Transformation journey: [TRANSFORMATION_JOURNEY]
- Obstacles overcome: [OBSTACLES_OVERCOME]
- Success achieved: [SUCCESS_ACHIEVED]
- New reality: [NEW_REALITY]

### Success Stories
- Customer background: [CUSTOMER_BACKGROUND]
- Initial situation: [INITIAL_SITUATION]
- Problem identification: [PROBLEM_IDENTIFICATION]
- Solution discovery: [SOLUTION_DISCOVERY]
- Implementation process: [IMPLEMENTATION_PROCESS]
- Results achieved: [RESULTS_ACHIEVED]
- Life impact: [LIFE_IMPACT]
- Testimonial elements: [TESTIMONIAL_ELEMENTS]

### Transformation Stories
- Before state: [BEFORE_STATE]
- Catalyst moment: [CATALYST_MOMENT]
- Change process: [CHANGE_PROCESS]
- Brand involvement: [BRAND_INVOLVEMENT]
- Milestone moments: [MILESTONE_MOMENTS]
- After state: [AFTER_STATE]
- Ongoing benefits: [ONGOING_BENEFITS]
- Ripple effects: [RIPPLE_EFFECTS]

### Product Stories
### Innovation Narratives
- Problem identification: [PRODUCT_PROBLEM_IDENTIFICATION]
- Research journey: [RESEARCH_JOURNEY]
- Development challenges: [DEVELOPMENT_CHALLENGES]
- Breakthrough moments: [PRODUCT_BREAKTHROUGH_MOMENTS]
- Testing phases: [TESTING_PHASES]
- Refinement process: [REFINEMENT_PROCESS]
- Launch story: [LAUNCH_STORY]
- Impact measurement: [IMPACT_MEASUREMENT]

Behind-the-Scenes:
- Development team: [DEVELOPMENT_TEAM]
- Creative process: [CREATIVE_PROCESS]
- Design iterations: [DESIGN_ITERATIONS]
- Quality standards: [QUALITY_STANDARDS]
- Craftsmanship details: [CRAFTSMANSHIP_DETAILS]
- Attention to detail: [ATTENTION_TO_DETAIL]
- Testing rigor: [TESTING_RIGOR]
- Excellence pursuit: [EXCELLENCE_PURSUIT]

### Cultural Stories
### Brand Culture
- Company culture: [COMPANY_CULTURE]
- Team dynamics: [TEAM_DYNAMICS]
- Work environment: [WORK_ENVIRONMENT]
- Collaboration style: [COLLABORATION_STYLE]
- Innovation culture: [INNOVATION_CULTURE]
- Diversity narrative: [DIVERSITY_NARRATIVE]
- Inclusion practices: [INCLUSION_PRACTICES]
- Cultural evolution: [CULTURAL_EVOLUTION]

### Community Stories
- Community building: [COMMUNITY_BUILDING]
- Shared experiences: [SHARED_EXPERIENCES]
- Collective achievements: [COLLECTIVE_ACHIEVEMENTS]
- Member spotlights: [MEMBER_SPOTLIGHTS]
- Community impact: [COMMUNITY_IMPACT]
- Relationship building: [RELATIONSHIP_BUILDING]
- Support networks: [SUPPORT_NETWORKS]
- Belonging creation: [BELONGING_CREATION]

### EMOTIONAL STORYTELLING

### Emotional Architecture
### Primary Emotions
- Core emotion: [CORE_EMOTION]
- Emotional journey: [EMOTIONAL_JOURNEY]
- Emotional triggers: [EMOTIONAL_TRIGGERS]
- Emotional peaks: [EMOTIONAL_PEAKS]
- Emotional resolution: [EMOTIONAL_RESOLUTION]
- Emotional memory: [EMOTIONAL_MEMORY]
- Emotional connection: [EMOTIONAL_CONNECTION]
- Emotional sustainability: [EMOTIONAL_SUSTAINABILITY]

### Emotional Techniques
- Empathy building: [EMPATHY_BUILDING]
- Vulnerability sharing: [VULNERABILITY_SHARING]
- Aspiration inspiration: [ASPIRATION_INSPIRATION]
- Fear addressing: [FEAR_ADDRESSING]
- Joy creation: [JOY_CREATION]
- Pride instillation: [PRIDE_INSTILLATION]
- Hope cultivation: [HOPE_CULTIVATION]
- Love expression: [LOVE_EXPRESSION]

### Sensory Storytelling
### Sensory Details
- Visual imagery: [VISUAL_IMAGERY]
- Auditory elements: [AUDITORY_ELEMENTS]
- Tactile descriptions: [TACTILE_DESCRIPTIONS]
- Olfactory references: [OLFACTORY_REFERENCES]
- Taste associations: [TASTE_ASSOCIATIONS]
- Kinesthetic feelings: [KINESTHETIC_FEELINGS]
- Atmospheric creation: [ATMOSPHERIC_CREATION]
- Environmental context: [ENVIRONMENTAL_CONTEXT]

### Memory Anchoring
- Memory triggers: [MEMORY_TRIGGERS]
- Nostalgia elements: [NOSTALGIA_ELEMENTS]
- Familiar references: [FAMILIAR_REFERENCES]
- Cultural touchstones: [CULTURAL_TOUCHSTONES]
- Personal connections: [PERSONAL_CONNECTIONS]
- Shared experiences: [SHARED_EXPERIENCES]
- Collective memories: [COLLECTIVE_MEMORIES]
- Timeless themes: [TIMELESS_THEMES]

### CONTENT STORYTELLING

### Content Strategy
Story-Driven Content:
- Content themes: [CONTENT_THEMES]
- Narrative threads: [NARRATIVE_THREADS]
- Story consistency: [STORY_CONSISTENCY]
- Character continuity: [CHARACTER_CONTINUITY]
- Plot development: [PLOT_DEVELOPMENT]
- Story evolution: [STORY_EVOLUTION]
- Audience engagement: [AUDIENCE_ENGAGEMENT]
- Content calendar: [STORY_CONTENT_CALENDAR]

Multi-Format Storytelling:
- Written content: [WRITTEN_CONTENT_APPROACH]
- Visual storytelling: [VISUAL_STORYTELLING_APPROACH]
- Video narratives: [VIDEO_NARRATIVE_APPROACH]
- Audio storytelling: [AUDIO_STORYTELLING_APPROACH]
- Interactive stories: [INTERACTIVE_STORY_APPROACH]
- Social media stories: [SOCIAL_MEDIA_STORY_APPROACH]
- Email storytelling: [EMAIL_STORYTELLING_APPROACH]
- Website narratives: [WEBSITE_NARRATIVE_APPROACH]

### Serial Storytelling
### Episode Structure
- Story arc planning: [STORY_ARC_PLANNING]
- Episode themes: [EPISODE_THEMES]
- Character development: [SERIAL_CHARACTER_DEVELOPMENT]
- Cliffhanger elements: [CLIFFHANGER_ELEMENTS]
- Continuity maintenance: [CONTINUITY_MAINTENANCE]
- Audience retention: [AUDIENCE_RETENTION]
- Story resolution: [STORY_RESOLUTION]
- Next chapter setup: [NEXT_CHAPTER_SETUP]

### Chapter Development
Chapter 1: [CHAPTER_1_TITLE]
- Chapter theme: [CHAPTER_1_THEME]
- Key events: [CHAPTER_1_EVENTS]
- Character focus: [CHAPTER_1_CHARACTER_FOCUS]
- Emotional arc: [CHAPTER_1_EMOTIONAL_ARC]
- Learning/insight: [CHAPTER_1_LEARNING]
- Connection to brand: [CHAPTER_1_BRAND_CONNECTION]
- Audience takeaway: [CHAPTER_1_TAKEAWAY]

Chapter 2: [CHAPTER_2_TITLE]
- Chapter theme: [CHAPTER_2_THEME]
- Key events: [CHAPTER_2_EVENTS]
- Character focus: [CHAPTER_2_CHARACTER_FOCUS]
- Emotional arc: [CHAPTER_2_EMOTIONAL_ARC]
- Learning/insight: [CHAPTER_2_LEARNING]
- Connection to brand: [CHAPTER_2_BRAND_CONNECTION]
- Audience takeaway: [CHAPTER_2_TAKEAWAY]

Chapter 3: [CHAPTER_3_TITLE]
- Chapter theme: [CHAPTER_3_THEME]
- Key events: [CHAPTER_3_EVENTS]
- Character focus: [CHAPTER_3_CHARACTER_FOCUS]
- Emotional arc: [CHAPTER_3_EMOTIONAL_ARC]
- Learning/insight: [CHAPTER_3_LEARNING]
- Connection to brand: [CHAPTER_3_BRAND_CONNECTION]
- Audience takeaway: [CHAPTER_3_TAKEAWAY]

### STORYTELLING APPLICATIONS

### Marketing Applications
### Campaign Integration
- Story-driven campaigns: [STORY_DRIVEN_CAMPAIGNS]
- Narrative consistency: [NARRATIVE_CONSISTENCY]
- Multi-touchpoint stories: [MULTI_TOUCHPOINT_STORIES]
- Cross-channel narratives: [CROSS_CHANNEL_NARRATIVES]
- Integrated messaging: [INTEGRATED_MESSAGING]
- Story amplification: [STORY_AMPLIFICATION]
- Narrative measurement: [NARRATIVE_MEASUREMENT]
- Story optimization: [STORY_OPTIMIZATION]

### Sales Enablement
- Sales narratives: [SALES_NARRATIVES]
- Pitch stories: [PITCH_STORIES]
- Case study narratives: [CASE_STUDY_NARRATIVES]
- Objection handling stories: [OBJECTION_HANDLING_STORIES]
- Value demonstration: [VALUE_DEMONSTRATION_STORIES]
- ROI narratives: [ROI_NARRATIVES]
- Success story templates: [SUCCESS_STORY_TEMPLATES]
- Relationship building: [RELATIONSHIP_BUILDING_STORIES]

### Internal Communications
### Culture Building
- Employee onboarding: [EMPLOYEE_ONBOARDING_STORIES]
- Culture reinforcement: [CULTURE_REINFORCEMENT_STORIES]
- Values demonstration: [VALUES_DEMONSTRATION_STORIES]
- Leadership narratives: [LEADERSHIP_NARRATIVES]
- Change management: [CHANGE_MANAGEMENT_STORIES]
- Success celebration: [SUCCESS_CELEBRATION_STORIES]
- Learning stories: [LEARNING_STORIES]
- Motivation narratives: [MOTIVATION_NARRATIVES]

### Stakeholder Communication
- Investor narratives: [INVESTOR_NARRATIVES]
- Partner stories: [PARTNER_STORIES]
- Vendor relations: [VENDOR_RELATION_STORIES]
- Media narratives: [MEDIA_NARRATIVES]
- Community stories: [COMMUNITY_STORIES]
- Regulatory narratives: [REGULATORY_NARRATIVES]
- Crisis communication: [CRISIS_COMMUNICATION_STORIES]
- Reputation management: [REPUTATION_MANAGEMENT_STORIES]

### STORY MEASUREMENT

### Impact Assessment
### Story Performance
- Engagement metrics: [STORY_ENGAGEMENT_METRICS]
- Emotional response: [EMOTIONAL_RESPONSE_METRICS]
- Brand recall: [BRAND_RECALL_METRICS]
- Message retention: [MESSAGE_RETENTION_METRICS]
- Share-ability: [SHAREABILITY_METRICS]
- Conversion impact: [CONVERSION_IMPACT_METRICS]
- Brand perception: [BRAND_PERCEPTION_METRICS]
- Loyalty metrics: [LOYALTY_METRICS]

### Feedback Collection
- Story feedback: [STORY_FEEDBACK_COLLECTION]
- Audience insights: [AUDIENCE_INSIGHTS_COLLECTION]
- Emotional tracking: [EMOTIONAL_TRACKING]
- Narrative testing: [NARRATIVE_TESTING]
- Content analysis: [CONTENT_ANALYSIS]
- Sentiment monitoring: [SENTIMENT_MONITORING]
- Cultural relevance: [CULTURAL_RELEVANCE_ASSESSMENT]
- Competitive analysis: [COMPETITIVE_NARRATIVE_ANALYSIS]

### Story Evolution
- Story refinement: [STORY_REFINEMENT]
- Narrative updates: [NARRATIVE_UPDATES]
- Character development: [CHARACTER_DEVELOPMENT_EVOLUTION]
- Plot progression: [PLOT_PROGRESSION]
- Audience adaptation: [AUDIENCE_ADAPTATION]
- Cultural alignment: [CULTURAL_ALIGNMENT]
- Relevance maintenance: [RELEVANCE_MAINTENANCE]
- Legacy building: [LEGACY_BUILDING]

### DESIGN OUTPUT
[Generate comprehensive brand storytelling framework with all specified elements]

Brand: [FINAL_BRAND_NAME]
Story Type: [FINAL_STORY_TYPE]

[COMPLETE_BRAND_STORYTELLING_SOLUTION]

---

### Storytelling Deliverables
- Core narratives: [NARRATIVE_COUNT]
- Character profiles: [CHARACTER_COUNT]
- Story variations: [STORY_VARIATION_COUNT]
- Content applications: [CONTENT_APPLICATION_COUNT]
- Voice guidelines: [VOICE_GUIDELINE_COUNT]
- Personality traits: [PERSONALITY_TRAIT_COUNT]

OUTPUT: Deliver comprehensive brand storytelling with:
1. Compelling narrative architecture
2. Distinctive brand voice and personality
3. Emotional storytelling framework
4. Multi-format story applications
5. Customer and product narratives
6. Internal and external story integration
7. Performance measurement system
```

## Variables
[All 400+ variables for comprehensive brand storytelling including narratives, voice, and personality]

## Usage Examples



## Best Practices

1. **Start with clear objectives** - Define what success looks like before beginning
2. **Use data to inform decisions** - Base choices on evidence and measurable outcomes
3. **Iterate and improve continuously** - Treat implementation as an ongoing process
4. **Engage stakeholders early** - Include key participants in planning and execution
5. **Document thoroughly** - Maintain clear records for reference and knowledge transfer
6. **Communicate regularly** - Keep all parties informed of progress and changes
7. **Address challenges proactively** - Identify potential issues before they become problems
8. **Celebrate milestones** - Recognize achievements to maintain motivation
9. **Learn from experience** - Reflect on what works and adjust accordingly
10. **Stay flexible** - Be ready to adapt based on feedback and changing circumstances

## Tips for Success

- Break complex tasks into manageable steps with clear milestones
- Set realistic timelines that account for dependencies and constraints
- Allocate sufficient resources including time, budget, and personnel
- Use templates and frameworks to ensure consistency and quality
- Seek feedback from users and stakeholders throughout the process
- Build in checkpoints to assess progress and make adjustments
- Maintain quality standards while remaining practical and efficient
- Document lessons learned for future reference and improvement
- Foster collaboration across teams and departments
- Stay current with industry best practices and emerging trends
### Example 1: Tech Startup Origin Story
```
STORY_TYPE: "Brand origin story"
BRAND_NAME: "InnovateTech Solutions"
TARGET_AUDIENCE: "B2B decision makers and tech professionals"
STORYTELLING_OBJECTIVE: "Establish credibility and inspire confidence"
COMPANY_STAGE: "Growth stage startup"
EMOTIONAL_GOAL: "Trust and excitement"
```

### Example 2: Consumer Brand Personality
```
STORY_TYPE: "Brand personality development"
BRAND_NAME: "EcoLife Products"
TARGET_AUDIENCE: "Environmentally conscious consumers"
STORYTELLING_OBJECTIVE: "Create emotional connection and brand loyalty"
COMPANY_STAGE: "Established brand expanding market"
EMOTIONAL_GOAL: "Hope and empowerment"
```

### Example 3: Customer Success Stories
```
STORY_TYPE: "Customer transformation stories"
BRAND_NAME: "FitnessPro Coaching"
TARGET_AUDIENCE: "Health and fitness seekers"
STORYTELLING_OBJECTIVE: "Demonstrate value and inspire action"
COMPANY_STAGE: "Mature service business"
EMOTIONAL_GOAL: "Inspiration and motivation"
```

## Customization Options

1. **Story Types**
   - Origin stories
   - Vision narratives
   - Customer stories
   - Product stories
   - Culture stories
   - Transformation stories
   - Purpose stories

2. **Industry Applications**
   - Technology/Software
   - Healthcare/Wellness
   - Financial services
   - E-commerce/Retail
   - Food & beverage
   - Professional services
   - Non-profit/Social

3. **Brand Archetypes**
   - Hero
   - Mentor/Sage
   - Rebel/Outlaw
   - Explorer/Pioneer
   - Creator/Artist
   - Caregiver/Nurturer
   - Ruler/Leader

4. **Communication Styles**
   - Professional/Corporate
   - Conversational/Friendly
   - Inspirational/Aspirational
   - Educational/Informative
   - Entertaining/Playful
   - Authentic/Transparent
   - Bold/Provocative