# Financial Analysis Template Generator

## Purpose
Create comprehensive financial analysis reports covering financial statements, financial ratios, and trend analysis to support business decision-making and performance evaluation.

## Template

```
You are a financial analyst specializing in financial statement analysis, ratio analysis, and business performance evaluation. Create a detailed financial analysis based on the following information:

Company Information:
- Company Name: [COMPANY_NAME]
- Industry: [INDUSTRY_SECTOR]
- Ticker Symbol: [STOCK_SYMBOL]
- Fiscal Year End: [FISCAL_YEAR_END]
- Analysis Period: [ANALYSIS_PERIOD]
- Company Size: [REVENUE_CLASSIFICATION]
- Business Model: [BUSINESS_MODEL]
- Geographic Presence: [GEOGRAPHIC_SCOPE]

Financial Statement Data:
- Current Year Revenue: [CURRENT_REVENUE]
- Prior Year Revenue: [PRIOR_REVENUE]
- Current Year Gross Profit: [CURRENT_GROSS_PROFIT]
- Current Year Operating Income: [CURRENT_OPERATING_INCOME]
- Current Year Net Income: [CURRENT_NET_INCOME]
- Total Assets: [TOTAL_ASSETS]
- Total Liabilities: [TOTAL_LIABILITIES]
- Shareholders' Equity: [SHAREHOLDERS_EQUITY]
- Cash and Cash Equivalents: [CASH_POSITION]
- Total Debt: [TOTAL_DEBT]

Operational Metrics:
- Number of Employees: [EMPLOYEE_COUNT]
- Key Customer Concentration: [CUSTOMER_CONCENTRATION]
- Market Share: [MARKET_POSITION]
- Seasonal Patterns: [SEASONALITY_FACTORS]
- Growth Stage: [BUSINESS_LIFECYCLE]
- Capital Intensity: [CAPITAL_REQUIREMENTS]
- Competitive Position: [COMPETITIVE_STANDING]
- Regulatory Environment: [REGULATORY_CONTEXT]

Analysis Objectives:
- Analysis Purpose: [ANALYSIS_OBJECTIVE]
- Key Stakeholders: [TARGET_AUDIENCE]
- Decision Context: [DECISION_FRAMEWORK]
- Benchmark Companies: [PEER_COMPANIES]
- Time Horizon: [ANALYSIS_TIMEFRAME]
- Risk Assessment Focus: [RISK_PRIORITIES]
- Performance Areas: [FOCUS_AREAS]
- Investment Perspective: [INVESTMENT_CONTEXT]

Market Context:
- Economic Environment: [ECONOMIC_CONDITIONS]
- Industry Trends: [INDUSTRY_DYNAMICS]
- Market Conditions: [MARKET_ENVIRONMENT]
- Interest Rate Environment: [INTEREST_RATES]
- Currency Factors: [CURRENCY_IMPACTS]
- Inflation Impact: [INFLATION_EFFECTS]
- Competitive Landscape: [COMPETITION_LEVEL]

Generate a comprehensive financial analysis that includes:

## EXECUTIVE SUMMARY
### Financial Performance Overview
- Overall financial health assessment
- Key performance highlights and concerns
- Profitability and efficiency evaluation
- Liquidity and solvency analysis
- Growth trends and sustainability
- Competitive position and market standing
- Risk factors and opportunities identification

### Key Findings and Recommendations
- Critical success factors and achievements
- Performance improvement opportunities
- Strategic recommendations and actions
- Investment thesis and valuation insights
- Risk mitigation priorities
- Future outlook and projections

## 1. COMPANY OVERVIEW AND BUSINESS MODEL
### 1.1 Business Description
- Company history and evolution
- Core business operations and segments
- Products and services portfolio
- Market positioning and competitive advantages
- Geographic presence and operations
- Customer base and market served
- Distribution channels and go-to-market strategy
- Key partnerships and relationships

### 1.2 Industry Analysis
- Industry dynamics and structure
- Market size and growth trends
- Key industry drivers and challenges
- Competitive landscape assessment
- Regulatory environment impact
- Technology trends and disruption
- Seasonal and cyclical patterns
- Barriers to entry and exit

### 1.3 Strategic Position
- Competitive advantages and moats
- Market share and positioning
- Brand strength and recognition
- Innovation and R&D capabilities
- Operational efficiency and scale
- Management quality and execution
- Strategic initiatives and priorities
- Risk factors and vulnerabilities

## 2. FINANCIAL STATEMENT ANALYSIS
### 2.1 Income Statement Analysis
#### Revenue Analysis
- Revenue growth trends and drivers
- Revenue composition by segment/product
- Geographic revenue distribution
- Customer concentration and dependency
- Revenue quality and sustainability
- Seasonal and cyclical patterns
- Market share gains or losses
- Pricing power and trends

#### Profitability Analysis
##### Gross Profit Analysis
- Gross margin trends and drivers
- Cost of goods sold analysis
- Product mix impact on margins
- Pricing and volume effects
- Supply chain and procurement efficiency
- Manufacturing and operational efficiency
- Competitive margin pressures
- Industry margin comparison

##### Operating Profit Analysis
- Operating margin trends and sustainability
- Operating expense control and efficiency
- SG&A expense analysis and trends
- R&D investment and productivity
- One-time and non-recurring items
- Operating leverage analysis
- Benchmark comparison with peers
- Margin expansion or compression factors

##### Net Income Analysis
- Net margin trends and quality
- Interest expense and debt service
- Tax rate analysis and optimization
- Non-operating income and expenses
- Earnings quality assessment
- Adjusted earnings calculations
- Earnings per share trends
- Dividend coverage and sustainability

### 2.2 Balance Sheet Analysis
#### Asset Analysis
##### Current Assets
- Cash and cash equivalents adequacy
- Accounts receivable quality and collection
- Inventory management and turnover
- Prepaid expenses and other assets
- Working capital management efficiency
- Liquidity position assessment
- Asset quality and realizability
- Current asset composition trends

##### Non-Current Assets
- Property, plant, and equipment investment
- Depreciation policies and asset utilization
- Intangible assets and amortization
- Investment portfolio evaluation
- Asset impairment and write-downs
- Capital allocation efficiency
- Asset productivity and returns
- Long-term asset strategy alignment

#### Liability and Equity Analysis
##### Current Liabilities
- Accounts payable management
- Short-term debt and credit facilities
- Accrued expenses and provisions
- Current portion of long-term debt
- Working capital obligations
- Liquidity pressure assessment
- Payment terms and relationships
- Current liability trends

##### Long-term Liabilities
- Long-term debt structure and terms
- Debt maturity profile and refinancing
- Interest rate exposure and hedging
- Pension and other post-employment benefits
- Deferred tax liabilities
- Off-balance-sheet obligations
- Credit rating and covenant compliance
- Debt capacity and flexibility

##### Shareholders' Equity
- Equity composition and structure
- Retained earnings and dividend policy
- Share buyback programs and impact
- Stock-based compensation effects
- Book value per share trends
- Return on equity analysis
- Equity dilution assessment
- Capital structure optimization

### 2.3 Cash Flow Statement Analysis
#### Operating Cash Flow
- Operating cash flow generation ability
- Cash conversion from earnings
- Working capital impact on cash flow
- Cash flow quality and sustainability
- Free cash flow calculation and trends
- Cash flow margin analysis
- Seasonal cash flow patterns
- Industry cash flow comparison

#### Investing Cash Flow
- Capital expenditure trends and adequacy
- Acquisition and divestiture activities
- Investment in intangible assets
- Strategic investment analysis
- Asset disposal and monetization
- Return on invested capital
- Capital allocation priorities
- Future investment requirements

#### Financing Cash Flow
- Debt issuance and repayment activities
- Equity financing and share transactions
- Dividend payments and policy
- Share buyback programs
- Interest payments and debt service
- Financing strategy and flexibility
- Capital structure management
- Cost of capital optimization

## 3. FINANCIAL RATIO ANALYSIS
### 3.1 Liquidity Ratios
#### Current Ratio
- Current ratio calculation and trends
- Industry benchmark comparison
- Seasonal variations and patterns
- Liquidity adequacy assessment
- Working capital management efficiency
- Short-term obligation coverage
- Cash conversion cycle analysis
- Liquidity risk evaluation

#### Quick Ratio (Acid-Test)
- Quick ratio calculation and interpretation
- Immediate liquidity assessment
- Inventory dependency evaluation
- Conservative liquidity measure
- Crisis liquidity preparedness
- Industry standard comparison
- Trend analysis and implications
- Management liquidity strategy

#### Cash Ratio
- Cash ratio calculation and significance
- Ultimate liquidity measure
- Cash management effectiveness
- Emergency liquidity capacity
- Debt service coverage ability
- Industry cash requirements
- Cash optimization opportunities
- Liquidity buffer adequacy

### 3.2 Efficiency Ratios
#### Asset Turnover Ratios
##### Total Asset Turnover
- Asset utilization efficiency
- Revenue generation per asset dollar
- Industry comparison and benchmarking
- Trend analysis and improvement
- Asset productivity optimization
- Capital efficiency measurement
- Management effectiveness indicator
- Strategic asset allocation

##### Inventory Turnover
- Inventory management efficiency
- Days in inventory calculation
- Industry benchmark comparison
- Seasonal adjustment factors
- Obsolescence and write-off risks
- Supply chain optimization
- Working capital efficiency
- Inventory strategy effectiveness

##### Accounts Receivable Turnover
- Collection efficiency measurement
- Days sales outstanding calculation
- Credit policy effectiveness
- Customer payment behavior
- Bad debt risk assessment
- Cash flow impact analysis
- Industry collection standards
- Credit management optimization

#### Working Capital Management
- Working capital turnover analysis
- Cash conversion cycle optimization
- Supplier payment management
- Customer financing strategy
- Seasonal working capital needs
- Industry working capital norms
- Management efficiency indicators
- Cash flow optimization opportunities

### 3.3 Leverage Ratios
#### Debt-to-Equity Ratio
- Capital structure analysis
- Leverage level assessment
- Industry leverage comparison
- Financial risk evaluation
- Debt capacity utilization
- Credit rating implications
- Covenant compliance status
- Optimal capital structure

#### Debt-to-Assets Ratio
- Asset financing structure
- Debt burden assessment
- Financial flexibility evaluation
- Risk profile analysis
- Industry standard comparison
- Trend analysis and implications
- Deleveraging opportunities
- Capital allocation strategy

#### Interest Coverage Ratio
- Debt service capability
- Interest payment security
- Financial stability indicator
- Credit risk assessment
- Earnings volatility impact
- Industry coverage standards
- Covenant requirement compliance
- Financial distress risk

### 3.4 Profitability Ratios
#### Return on Assets (ROA)
- Asset productivity measurement
- Management effectiveness indicator
- Industry profitability comparison
- Trend analysis and drivers
- Asset utilization optimization
- Business model efficiency
- Capital allocation success
- Competitive advantage assessment

#### Return on Equity (ROE)
- Shareholder value creation
- Equity investment returns
- Leverage impact on returns
- Industry ROE comparison
- Sustainable growth rate
- Management performance measure
- Investor expectation alignment
- Value creation assessment

#### DuPont Analysis
- ROE decomposition analysis
- Profit margin contribution
- Asset turnover impact
- Leverage effect analysis
- Performance driver identification
- Improvement opportunity assessment
- Strategic focus area identification
- Comprehensive performance evaluation

### 3.5 Market Valuation Ratios
#### Price-to-Earnings (P/E) Ratio
- Market valuation assessment
- Investor expectation analysis
- Industry valuation comparison
- Growth expectation implications
- Earnings quality considerations
- Forward P/E analysis
- Valuation multiple trends
- Investment attractiveness

#### Price-to-Book (P/B) Ratio
- Asset-based valuation measure
- Book value accuracy assessment
- Market premium or discount
- Asset-heavy business evaluation
- Liquidation value considerations
- Industry asset valuation
- Market sentiment indicator
- Value investment screening

#### Enterprise Value Ratios
- EV/Revenue multiple analysis
- EV/EBITDA comparison
- Capital structure neutral valuation
- Acquisition valuation reference
- Industry multiple comparison
- Growth-adjusted valuations
- Strategic value assessment
- Investment decision support

## 4. TREND ANALYSIS AND FORECASTING
### 4.1 Historical Trend Analysis
#### Revenue Growth Analysis
- Multi-year revenue trend evaluation
- Growth rate consistency and volatility
- Market share evolution
- Competitive position changes
- Industry growth rate comparison
- Economic cycle impact assessment
- Seasonal trend identification
- Growth driver sustainability

#### Profitability Trend Analysis
- Margin expansion or compression trends
- Cost structure evolution
- Operating leverage effects
- Efficiency improvement tracking
- Industry margin comparison
- Profitability sustainability assessment
- Margin volatility and drivers
- Competitive profitability dynamics

#### Balance Sheet Trends
- Asset composition evolution
- Liability structure changes
- Capital structure optimization
- Working capital trends
- Investment pattern analysis
- Financing strategy evolution
- Leverage trend assessment
- Equity structure development

### 4.2 Peer Comparison Analysis
#### Industry Benchmarking
- Key metric comparison with peers
- Relative performance assessment
- Market position evaluation
- Competitive advantage identification
- Performance gap analysis
- Best practice identification
- Industry ranking and positioning
- Peer group selection rationale

#### Best-in-Class Analysis
- Top performer identification
- Success factor analysis
- Performance gap quantification
- Improvement opportunity assessment
- Strategic initiative benchmarking
- Operational excellence comparison
- Financial metric leadership
- Competitive advantage sustainability

### 4.3 Forward-Looking Analysis
#### Financial Projections
- Revenue growth forecasting
- Profitability projection
- Cash flow forecasting
- Balance sheet projections
- Key ratio forecasting
- Scenario analysis development
- Sensitivity analysis
- Risk-adjusted projections

#### Valuation Analysis
- Discounted cash flow modeling
- Comparable company analysis
- Sum-of-the-parts valuation
- Asset-based valuation
- Market multiple application
- Risk premium assessment
- Terminal value calculation
- Valuation range determination

## 5. RISK ANALYSIS
### 5.1 Financial Risk Assessment
#### Credit Risk
- Default probability assessment
- Credit rating analysis
- Debt service capability
- Liquidity risk evaluation
- Covenant compliance monitoring
- Financial distress indicators
- Recovery value estimation
- Credit enhancement features

#### Market Risk
- Interest rate exposure
- Foreign exchange risk
- Commodity price exposure
- Equity market correlation
- Systematic risk factors
- Market volatility impact
- Hedging strategy effectiveness
- Economic sensitivity analysis

#### Operational Risk
- Business model sustainability
- Industry disruption risk
- Competitive threat assessment
- Regulatory compliance risk
- Key person dependency
- Operational disruption risk
- Technology and cyber risk
- Environmental and social risk

### 5.2 Scenario Analysis
#### Base Case Scenario
- Most likely outcome projection
- Key assumption documentation
- Performance expectation setting
- Risk factor consideration
- Market condition assumptions
- Management strategy execution
- Industry trend continuation
- Economic environment stability

#### Optimistic Scenario
- Best case outcome modeling
- Growth acceleration factors
- Market expansion opportunities
- Competitive advantage exploitation
- Operational efficiency gains
- Market condition improvements
- Strategic initiative success
- Economic tailwind benefits

#### Pessimistic Scenario
- Downside risk quantification
- Economic downturn impact
- Competitive pressure increase
- Market contraction effects
- Operational challenge magnification
- Financial stress testing
- Liquidity crunch analysis
- Recovery strategy requirements

## 6. INVESTMENT RECOMMENDATIONS
### 6.1 Investment Thesis
- Compelling investment rationale
- Value creation opportunities
- Competitive advantage sustainability
- Growth potential assessment
- Risk-return profile evaluation
- Catalyst identification and timing
- Downside protection analysis
- Long-term value proposition

### 6.2 Strategic Recommendations
#### Operational Improvements
- Efficiency enhancement opportunities
- Cost reduction initiatives
- Revenue growth strategies
- Market expansion possibilities
- Product development priorities
- Technology investment needs
- Organizational capability building
- Performance optimization areas

#### Financial Optimization
- Capital structure improvement
- Working capital optimization
- Cash flow enhancement
- Dividend policy optimization
- Share buyback considerations
- Debt refinancing opportunities
- Tax optimization strategies
- Capital allocation priorities

### 6.3 Risk Mitigation
- Key risk identification and prioritization
- Risk mitigation strategy development
- Monitoring and early warning systems
- Contingency planning requirements
- Insurance and hedging strategies
- Diversification opportunities
- Operational risk controls
- Financial risk management

## APPENDICES
- Detailed financial statements
- Ratio calculation methodologies
- Peer comparison data
- Industry benchmark information
- Economic and market data
- Management discussion analysis
- Audit report highlights
- Supplementary financial disclosures

Ensure the financial analysis is:
- Comprehensive and objective
- Data-driven and analytically rigorous
- Forward-looking and strategic
- Risk-aware and realistic
- Actionable and decision-focused
- Industry-contextualized
- Stakeholder-appropriate
```

## Variables
- `[COMPANY_NAME]`: Company being analyzed
- `[INDUSTRY_SECTOR]`: Industry classification
- `[STOCK_SYMBOL]`: Ticker symbol if public
- `[FISCAL_YEAR_END]`: Fiscal year end date
- `[ANALYSIS_PERIOD]`: Time period for analysis
- `[REVENUE_CLASSIFICATION]`: Company size by revenue
- `[BUSINESS_MODEL]`: Business operating model
- `[GEOGRAPHIC_SCOPE]`: Geographic presence
- `[CURRENT_REVENUE]`: Current year revenue
- `[PRIOR_REVENUE]`: Previous year revenue
- `[CURRENT_GROSS_PROFIT]`: Current gross profit
- `[CURRENT_OPERATING_INCOME]`: Operating income
- `[CURRENT_NET_INCOME]`: Net income
- `[TOTAL_ASSETS]`: Total asset value
- `[TOTAL_LIABILITIES]`: Total liabilities
- `[SHAREHOLDERS_EQUITY]`: Equity value
- `[CASH_POSITION]`: Cash and equivalents
- `[TOTAL_DEBT]`: Total debt outstanding
- `[EMPLOYEE_COUNT]`: Number of employees
- `[CUSTOMER_CONCENTRATION]`: Customer concentration risk
- `[MARKET_POSITION]`: Market share
- `[SEASONALITY_FACTORS]`: Seasonal patterns
- `[BUSINESS_LIFECYCLE]`: Growth stage
- `[CAPITAL_REQUIREMENTS]`: Capital intensity
- `[COMPETITIVE_STANDING]`: Competitive position
- `[REGULATORY_CONTEXT]`: Regulatory environment
- `[ANALYSIS_OBJECTIVE]`: Purpose of analysis
- `[TARGET_AUDIENCE]`: Key stakeholders
- `[DECISION_FRAMEWORK]`: Decision context
- `[PEER_COMPANIES]`: Benchmark companies
- `[ANALYSIS_TIMEFRAME]`: Time horizon
- `[RISK_PRIORITIES]`: Risk assessment focus
- `[FOCUS_AREAS]`: Performance areas
- `[INVESTMENT_CONTEXT]`: Investment perspective
- `[ECONOMIC_CONDITIONS]`: Economic environment
- `[INDUSTRY_DYNAMICS]`: Industry trends
- `[MARKET_ENVIRONMENT]`: Market conditions
- `[INTEREST_RATES]`: Interest rate environment
- `[CURRENCY_IMPACTS]`: Currency factors
- `[INFLATION_EFFECTS]`: Inflation impact
- `[COMPETITION_LEVEL]`: Competitive landscape

## Usage Example
Use for investment analysis, credit assessment, performance evaluation, due diligence, strategic planning, or financial reporting and communication.

## Customization Tips
- Include industry-specific metrics and ratios
- Add relevant regulatory and compliance factors
- Consider economic and market cycle impacts
- Include ESG and sustainability factors
- Adjust analysis depth to decision requirements
- Consider stakeholder information needs and sophistication