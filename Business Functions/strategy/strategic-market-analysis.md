---
title: Market Analysis Template Generator
category: strategy
tags:
- ai-ml
- research
- strategy
use_cases:
- Creating comprehensive market analysis reports covering competitive analysis, SWOT
  analysis, and market trends to inform strategic decision-making.
- Project planning and execution
- Strategy development
related_templates:
- strategy/okr-implementation-framework.md
- strategy/digital-transformation-roadmap.md
- operations/lean-six-sigma-implementation.md
last_updated: 2025-11-09
industries:
- finance
- government
- healthcare
- manufacturing
- retail
- technology
type: template
difficulty: intermediate
slug: strategic-market-analysis
---

# Market Analysis Template Generator

## Purpose
Create comprehensive market analysis reports covering competitive analysis, SWOT analysis, and market trends to inform strategic decision-making.

## Quick Strategic Market Analysis Prompt
Conduct market analysis for [COMPANY_NAME] in [INDUSTRY] with [MARKET_POSITION] (leader/challenger/niche) position. Analyze TAM of [TAM_SIZE] growing at [GROWTH_RATE]% CAGR. Profile [COMPETITOR_COUNT] key competitors on positioning, strengths, weaknesses, and market share. Segment customers by [SEGMENTATION_CRITERIA]. Identify [TREND_COUNT] key market trends and disruption risks. Assess opportunities in [TARGET_MARKETS] markets. Deliver strategic recommendations for [STRATEGIC_GOALS] (market share growth/expansion/differentiation).

## Quick Start

### Minimal Example
```
Company Name: DataCloud Solutions
Industry: Cloud Analytics SaaS
Market Position: Challenger (15% market share)
Target Markets: North America, Europe
TAM: $45B, Growth Rate: 22% CAGR
```

### When to Use This
- Conducting market sizing and opportunity assessment for new market entry or expansion
- Analyzing competitive landscape with detailed competitor profiling and positioning analysis
- Performing comprehensive SWOT analysis to identify strategic priorities and opportunities
- Evaluating customer segments, needs, and journey to inform go-to-market strategy

### Basic 3-Step Workflow
1. Analyze market size (TAM/SAM/SOM), growth trends, segmentation, and industry dynamics with drivers and restraints
2. Profile competitors (market leaders and emerging players), assess positioning, strategies, strengths/weaknesses, and create competitive maps
3. Conduct SWOT analysis, assess customer segments and needs, prioritize opportunities, and develop strategic recommendations with implementation roadmap

**Time to complete:** 3-5 hours for comprehensive market analysis; 2-4 weeks with primary research

## Template

```
You are a market research analyst specializing in competitive intelligence, SWOT analysis, and market trend analysis. Create a detailed market analysis based on the following information:

Company Context:
- Company Name: [COMPANY_NAME]
- Industry: [INDUSTRY_SECTOR]
- Market Position: [CURRENT_POSITION]
- Geographic Focus: [TARGET_MARKETS]
- Business Model: [BUSINESS_MODEL]
- Products/Services: [OFFERINGS]

### Market Overview
- Market Size (TAM): [TOTAL_ADDRESSABLE_MARKET]
- Market Size (SAM): [SERVICEABLE_ADDRESSABLE_MARKET]
- Market Size (SOM): [SERVICEABLE_OBTAINABLE_MARKET]
- Market Growth Rate: [ANNUAL_GROWTH_RATE]
- Market Maturity: [EMERGING/GROWING/MATURE/DECLINING]
- Key Market Segments: [MARKET_SEGMENTS]
- Geographic Regions: [REGIONAL_MARKETS]
- Market Drivers: [GROWTH_DRIVERS]

### Competitive Landscape
- Market Leader: [MARKET_LEADER]
- Top 3 Competitors: [TOP_COMPETITORS]
- Indirect Competitors: [INDIRECT_COMPETITION]
- New Entrants: [EMERGING_PLAYERS]
- Market Share Distribution: [SHARE_BREAKDOWN]
- Competitive Intensity: [COMPETITION_LEVEL]
- Barrier to Entry: [ENTRY_BARRIERS]
- Switching Costs: [CUSTOMER_SWITCHING]

### Customer Analysis
- Primary Customer Segments: [CUSTOMER_SEGMENTS]
- Customer Demographics: [DEMOGRAPHICS]
- Customer Psychographics: [PSYCHOGRAPHICS]
- Customer Needs: [CUSTOMER_NEEDS]
- Pain Points: [CUSTOMER_PAIN_POINTS]
- Buying Process: [PURCHASE_JOURNEY]
- Decision Criteria: [SELECTION_FACTORS]
- Price Sensitivity: [PRICE_ELASTICITY]

### Industry Trends
- Technology Trends: [TECH_TRENDS]
- Regulatory Changes: [REGULATORY_SHIFTS]
- Economic Factors: [ECONOMIC_IMPACT]
- Social Changes: [SOCIAL_TRENDS]
- Environmental Factors: [ENVIRONMENTAL_IMPACT]
- Disruption Threats: [POTENTIAL_DISRUPTIONS]
- Innovation Opportunities: [INNOVATION_AREAS]

### Internal Assessment
- Company Strengths: [INTERNAL_STRENGTHS]
- Company Weaknesses: [INTERNAL_WEAKNESSES]
- Core Capabilities: [KEY_CAPABILITIES]
- Resource Limitations: [RESOURCE_CONSTRAINTS]
- Operational Efficiency: [EFFICIENCY_LEVELS]
- Financial Position: [FINANCIAL_HEALTH]
- Brand Recognition: [BRAND_STRENGTH]
- Market Share: [CURRENT_SHARE]

Generate a comprehensive market analysis report that includes:

## EXECUTIVE SUMMARY
### Key Findings
- Market opportunity assessment
- Competitive position analysis
- Strategic recommendations
- Investment implications
- Risk assessment summary
- Growth potential evaluation

### Strategic Recommendations
- Priority actions for market positioning
- Competitive response strategies
- Market entry or expansion opportunities
- Investment priorities
- Risk mitigation approaches

## 1. MARKET OVERVIEW AND SIZING
### 1.1 Market Definition
- Market scope and boundaries
- Product/service categories included
- Geographic coverage
- Time horizon for analysis

### 1.2 Market Size Analysis
- Total Addressable Market (TAM) calculation
- Serviceable Addressable Market (SAM) assessment
- Serviceable Obtainable Market (SOM) estimation
- Market sizing methodology and assumptions
- Historical market growth trends
- Future growth projections and scenarios

### 1.3 Market Segmentation
- Primary segmentation criteria
- Segment size and growth rates
- Segment attractiveness analysis
- Cross-segment dynamics
- Emerging segments identification
- Segment-specific opportunities

### 1.4 Geographic Analysis
- Regional market variations
- Geographic growth patterns
- Market penetration by region
- Regulatory differences by geography
- Cultural and economic factors
- Expansion opportunities assessment

## 2. INDUSTRY DYNAMICS AND TRENDS
### 2.1 Industry Structure Analysis
- Value chain mapping
- Industry concentration levels
- Vertical integration patterns
- Distribution channel analysis
- Industry profitability trends
- Key success factors

### 2.2 Market Drivers and Restraints
#### Growth Drivers
- Technology advancement impacts
- Demographic trend influences
- Economic factor contributions
- Regulatory supportive changes
- Innovation and disruption opportunities
- Customer behavior evolution

#### Market Restraints
- Economic headwinds and challenges
- Regulatory constraints and compliance
- Technology limitations and barriers
- Resource scarcity and cost pressures
- Competitive intensity impacts
- Market saturation factors

### 2.3 Trend Analysis
#### Technology Trends
- Emerging technology adoption
- Digital transformation impacts
- Automation and AI integration
- Platform and ecosystem development
- Data analytics and insights
- Innovation pace and direction

#### Regulatory Trends
- Policy changes and implications
- Compliance requirement evolution
- International trade impacts
- Environmental regulations
- Data privacy and security rules
- Industry-specific regulations

#### Social and Economic Trends
- Consumer behavior shifts
- Generational preference changes
- Economic cycle impacts
- Labor market dynamics
- Sustainability and ESG factors
- Globalization effects

## 3. COMPETITIVE LANDSCAPE ANALYSIS
### 3.1 Competitor Identification and Profiling
#### Market Leaders
For each major competitor, analyze:
- Company background and history
- Market share and position
- Financial performance metrics
- Product/service portfolio
- Geographic presence
- Strategic focus and priorities

#### Emerging Players
- New entrant analysis
- Disruptive business models
- Technology-driven competitors
- International expansion players
- Niche specialists
- Startup ecosystem activity

### 3.2 Competitive Positioning Analysis
- Competitive mapping framework
- Price-value positioning
- Feature and benefit comparison
- Brand strength assessment
- Customer loyalty evaluation
- Market share trends

### 3.3 Competitor Strategies Analysis
#### Growth Strategies
- Market penetration approaches
- Product development initiatives
- Market expansion plans
- Acquisition and partnership strategies
- Innovation and R&D investments
- Digital transformation efforts

#### Competitive Tactics
- Pricing strategies and changes
- Product launch patterns
- Marketing and promotion approaches
- Distribution channel strategies
- Customer acquisition methods
- Retention and loyalty programs

### 3.4 Competitive Strengths and Weaknesses
- Core competency analysis
- Resource and capability comparison
- Operational efficiency assessment
- Innovation capacity evaluation
- Brand and reputation analysis
- Financial strength comparison

## 4. CUSTOMER ANALYSIS
### 4.1 Customer Segmentation
#### Primary Segments
- Demographic characteristics
- Geographic distribution
- Behavioral patterns
- Needs and preferences
- Value drivers and priorities
- Purchase decision factors

#### Secondary Segments
- Emerging customer groups
- Underserved segments
- High-growth segments
- Premium segments
- Price-sensitive segments
- Loyalty segment analysis

### 4.2 Customer Journey Analysis
- Awareness stage dynamics
- Consideration phase factors
- Purchase decision process
- Post-purchase experience
- Retention and loyalty drivers
- Advocacy and referral patterns

### 4.3 Customer Needs Assessment
- Functional requirements
- Emotional drivers
- Social influences
- Economic considerations
- Convenience factors
- Quality expectations

### 4.4 Customer Behavior Trends
- Channel preference evolution
- Technology adoption patterns
- Price sensitivity changes
- Brand loyalty trends
- Service expectation increases
- Sustainability preferences

## 5. SWOT ANALYSIS
### 5.1 Strengths Analysis
#### Internal Advantages
- Core competencies and capabilities
- Unique resources and assets
- Strong brand recognition
- Financial stability and resources
- Operational excellence
- Innovation and R&D capabilities
- Strategic partnerships
- Market position advantages

#### Competitive Advantages
- Cost leadership factors
- Differentiation capabilities
- First-mover advantages
- Exclusive relationships
- Proprietary technology
- Scale and scope benefits
- Network effects
- Customer loyalty

### 5.2 Weaknesses Analysis
#### Internal Limitations
- Resource constraints
- Capability gaps
- Operational inefficiencies
- Financial limitations
- Brand perception issues
- Innovation deficiencies
- Organizational challenges
- System and process gaps

#### Competitive Disadvantages
- Market position weaknesses
- Cost disadvantages
- Technology gaps
- Distribution limitations
- Customer acquisition challenges
- Retention difficulties
- Scale disadvantages
- Geographic limitations

### 5.3 Opportunities Analysis
#### Market Opportunities
- Underserved customer segments
- Geographic expansion potential
- New product/service categories
- Emerging technology applications
- Partnership and alliance possibilities
- Acquisition targets
- Market consolidation opportunities
- Regulatory favorable changes

#### Growth Opportunities
- Market share gain potential
- Premium positioning opportunities
- Cross-selling possibilities
- Up-selling potential
- New channel development
- International expansion
- Digital transformation benefits
- Innovation commercialization

### 5.4 Threats Analysis
#### External Threats
- New competitor entry
- Substitute product/service risks
- Technology disruption potential
- Economic downturn impacts
- Regulatory unfavorable changes
- Supply chain disruptions
- Talent shortage risks
- Cybersecurity vulnerabilities

#### Competitive Threats
- Price war potential
- Market share erosion risks
- Customer defection threats
- Innovation gaps
- Brand reputation risks
- Channel conflict possibilities
- Partnership vulnerabilities
- Capacity limitations

## 6. MARKET OPPORTUNITY ASSESSMENT
### 6.1 Opportunity Prioritization Matrix
- Market attractiveness evaluation
- Competitive position assessment
- Resource requirement analysis
- Risk-return evaluation
- Timeline considerations
- Strategic fit alignment

### 6.2 Market Entry/Expansion Strategy
- Entry mode selection
- Go-to-market approach
- Resource allocation recommendations
- Partnership strategy
- Investment requirements
- Success metrics definition

### 6.3 Innovation Opportunities
- Product/service innovation areas
- Business model innovation potential
- Technology application opportunities
- Process innovation possibilities
- Customer experience enhancements
- Sustainability innovations

## 7. STRATEGIC RECOMMENDATIONS
### 7.1 Strategic Priorities
- Market positioning recommendations
- Competitive response strategies
- Growth acceleration approaches
- Defensive strategies
- Investment priorities
- Risk mitigation plans

### 7.2 Implementation Roadmap
- Short-term actions
- Medium-term initiatives
- Long-term strategic moves (18+ months)
- Resource allocation plans
- Success metrics and KPIs
- Milestone tracking approach

### 7.3 Risk Management
- Key risk identification
- Risk assessment and prioritization
- Mitigation strategies
- Contingency planning
- Monitoring and early warning systems
- Response protocols

## 8. FINANCIAL IMPLICATIONS
### 8.1 Market Opportunity Valuation
- Revenue potential estimation
- Market share assumptions
- Pricing strategy implications
- Cost structure considerations
- Investment requirements
- Return on investment projections

### 8.2 Sensitivity Analysis
- Key assumption testing
- Scenario planning
- Risk-adjusted returns
- Break-even analysis
- Sensitivity to market changes
- Economic impact assessment

## APPENDICES
- Detailed competitor profiles
- Market research data sources
- Financial models and calculations
- Survey results and customer feedback
- Industry expert interviews
- Regulatory analysis details
- Technology trend research
- Geographic market assessments

### Ensure the market analysis is
- Data-driven with credible sources
- Objective and unbiased
- Actionable with clear recommendations
- Comprehensive yet focused
- Visually supported with charts/graphs
- Regularly updatable
- Aligned with business objectives
```

## Variables
- `[COMPANY_NAME]`: Company being analyzed
- `[INDUSTRY_SECTOR]`: Primary industry classification
- `[CURRENT_POSITION]`: Current market position
- `[TARGET_MARKETS]`: Geographic markets of interest
- `[BUSINESS_MODEL]`: Revenue generation approach
- `[OFFERINGS]`: Products and services offered
- `[TOTAL_ADDRESSABLE_MARKET]`: Total market size
- `[SERVICEABLE_ADDRESSABLE_MARKET]`: Addressable portion
- `[SERVICEABLE_OBTAINABLE_MARKET]`: Obtainable market share
- `[ANNUAL_GROWTH_RATE]`: Market growth percentage
- `[EMERGING/GROWING/MATURE/DECLINING]`: Market maturity stage
- `[MARKET_SEGMENTS]`: Key market segments
- `[REGIONAL_MARKETS]`: Geographic regions covered
- `[GROWTH_DRIVERS]`: Factors driving market growth
- `[MARKET_LEADER]`: Current market leader
- `[TOP_COMPETITORS]`: Main competitors list
- `[INDIRECT_COMPETITION]`: Indirect competitors
- `[EMERGING_PLAYERS]`: New market entrants
- `[SHARE_BREAKDOWN]`: Market share distribution
- `[COMPETITION_LEVEL]`: Competitive intensity level
- `[ENTRY_BARRIERS]`: Barriers to market entry
- `[CUSTOMER_SWITCHING]`: Customer switching costs
- `[CUSTOMER_SEGMENTS]`: Primary customer groups
- `[DEMOGRAPHICS]`: Customer demographic profile
- `[PSYCHOGRAPHICS]`: Customer psychographic data
- `[CUSTOMER_NEEDS]`: Primary customer needs
- `[CUSTOMER_PAIN_POINTS]`: Customer challenges
- `[PURCHASE_JOURNEY]`: Customer buying process
- `[SELECTION_FACTORS]`: Decision criteria
- `[PRICE_ELASTICITY]`: Price sensitivity level
- `[TECH_TRENDS]`: Technology trend impacts
- `[REGULATORY_SHIFTS]`: Regulatory changes
- `[ECONOMIC_IMPACT]`: Economic factor effects
- `[SOCIAL_TRENDS]`: Social change impacts
- `[ENVIRONMENTAL_IMPACT]`: Environmental considerations
- `[POTENTIAL_DISRUPTIONS]`: Disruption threats
- `[INNOVATION_AREAS]`: Innovation opportunities
- `[INTERNAL_STRENGTHS]`: Company strengths
- `[INTERNAL_WEAKNESSES]`: Company weaknesses
- `[KEY_CAPABILITIES]`: Core capabilities
- `[RESOURCE_CONSTRAINTS]`: Resource limitations
- `[EFFICIENCY_LEVELS]`: Operational efficiency
- `[FINANCIAL_HEALTH]`: Financial position
- `[BRAND_STRENGTH]`: Brand recognition level
- `[CURRENT_SHARE]`: Current market share

## Usage Example
Use this template for strategic planning, investment decisions, competitive intelligence, market entry planning, or business development initiatives.

## Customization Tips
- Include industry-specific metrics and benchmarks
- Add visual competitive positioning maps
- Incorporate primary research data when available
- Tailor depth of analysis to decision requirements
- Update regularly as market conditions change
- Consider cultural factors for international markets